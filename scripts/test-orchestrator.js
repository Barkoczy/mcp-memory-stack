...
class TestOrchestrator {
  ...
  async stopAllContainers() {
    ...
    try {
      ...
      const mcpContainers = containers.filter(
        name =>
          name.includes('mcp') ||
  ...
  async startContainers() {
    ...
    const composeResult = await this.executeCommand('docker-compose up -d', { timeout: 120000 });
  ...
  _calculateTestStats() {
    ...
    return { totalErrors, totalWarnings, passedSuites, totalSuites };
  }

  _logTestSummary(report) {
    this.log('='.repeat(80));
  ...
  _logErrorsAndWarnings(totalErrors, totalWarnings) {
    if (totalErrors > 0) {
      this.log('\nERRORS FOUND:');
  ...
  generateReport() {
    ...
    const { totalErrors, totalWarnings, passedSuites, totalSuites } = this._calculateTestStats();
    ...
    this._logTestSummary(report);
    this.log(`Report saved to: ${reportPath}`);
    this.log(`Logs saved to: ${this.logFile}`);

    this._logErrorsAndWarnings(totalErrors, totalWarnings);
    this.log('='.repeat(80));
